...
#	pragma warning(pop)
		...
		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<!Common::Helpers::ConfirmAllowed<Ftype>::dimensions, bool>::type Client::opPublishMessage(const Common::JString& channelName, Ftype parameters)
		    Sends a message to the specified public channel.
		    @param channelName channel name
		    @param parameters message to send
		    @returns true if the message has been sent
		 */

		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<Common::Helpers::ConfirmAllowed<Ftype>::dimensions==1, bool>::type Client::opPublishMessage(const Common::JString& channelName, Ftype pParameterArray, typename Common::Helpers::ArrayLengthType<Ftype>::type arrSize)
		    @overload
		    @param channelName channel name
		    @param pParameterArray message to send
			@param arrSize the number of elements in pParameterArray
		 */

		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<(Common::Helpers::ConfirmAllowed<Ftype>::dimensions>1), bool>::type Client::opPublishMessage(const Common::JString& channelName, Ftype pParameterArray, const short* pArrSizes)
			@overload
		    @param channelName channel name
		    @param pParameterArray message to send
		...
		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<!Common::Helpers::ConfirmAllowed<Ftype>::dimensions, bool>::type Client::opSendPrivateMessage(const Common::JString& userName, Ftype parameters, bool encrypt=false)
		    ...
		    @param channelName user name
			@param parameters message to send
			@param encrypt true to send the message encrypted, false (default) to send it unencrypted
		    @returns true if the message has been sent
		 */

		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<Common::Helpers::ConfirmAllowed<Ftype>::dimensions==1, bool>::type Client::opSendPrivateMessage(const Common::JString& userName, Ftype pParameterArray, typename Common::Helpers::ArrayLengthType<Ftype>::type arrSize, bool encrypt=false)
		    @overload
		...
		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<(Common::Helpers::ConfirmAllowed<Ftype>::dimensions>1), bool>::type Client::opSendPrivateMessage(const Common::JString& userName, Ftype pParameterArray, const short* pArrSizes, bool encrypt=false)
			@overload
		    @param userName user name
		    @param pParameterArray message to send
			@param pArrSizes an array holding the number of elements for each dimension of pParameterArray
		...
		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<!Common::Helpers::ConfirmAllowed<Ftype>::dimensions, bool>::type Client::opSetOnlineStatus(int status, Ftype parameters=Ftype(), bool skipMessage=false)
		    ...
		    @param parameters optional message
		    @param skipMessage If true, the message gets ignored. It can be empty but won't replace any current message.
		    @returns true if the command has been sent
		 */

		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<Common::Helpers::ConfirmAllowed<Ftype>::dimensions==1, bool>::type Client::opSetOnlineStatus(int status, Ftype pParameterArray=NULL, typename Common::Helpers::ArrayLengthType<Ftype>::type arrSize=0, bool skipMessage=false)
		    @overload
		    @param status predefined states are in class UserStatus. Other values can be used at will
		    @param pParameterArray optional message
			@param arrSize the number of elements in pParameterArray
			@param skipMessage If true, the message gets ignored. It can be empty but won't replace any current message.
		 */

		/** @fn template<typename Ftype> typename Common::Helpers::EnableIf<(Common::Helpers::ConfirmAllowed<Ftype>::dimensions>1), bool>::type Client::opSetOnlineStatus(int status, Ftype pParameterArray=NULL, const short* pArrSizes=NULL, bool skipMessage=false)
		    @overload
		    @param status predefined states are in class UserStatus. Other values can be used at will
		    @param pParameterArray optional message
			@param pArrSizes an array holding the number of elements for each dimension of pParameterArray
