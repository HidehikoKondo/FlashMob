...
Console::Console(void)
	: mImp(*new ConsoleImplementation)
{
}
...
void DemoScene::menuConnectCallback(CCObject* pSender)
{
	...
	mpClient->connect(ExitGames::Chat::AuthenticationValues().setUserID(mUserID));
}
...
void DemoScene::menuDisconnectCallback(CCObject* pSender)
{
	...
	if(mpClient->getState() == ExitGames::Chat::ClientState::Uninitialized || mpClient->getState() == ExitGames::Chat::ClientState::Disconnected) 
...
void DemoScene::menuUserStatusCallback(CCObject* pSender)
{
	...
	if(mpClient->opSetOnlineStatus(item->getTag(), L"My status changed")) // update message
		Console::get().writeLine(L"[i] my status sent: " + item->getTag());
}

void DemoScene::sendMessage(void) 
{
	ExitGames::Common::JString s = mpMessageInput->getString();
	int index;
	ExitGames::Common::JString sep = s.substring((index=s.indexOf(L':'))<0?index:index+1);
	...
	{
		static int cnt = 0;
		const ExitGames::Common::JVector<ExitGames::Chat::Channel*>& chs = mpClient->getPublicChannels();
		if(chs.getSize())
...
void DemoScene::debugReturn(int debugLevel, const ExitGames::Common::JString& string)
{
...
void DemoScene::onStateChange(int state)
{
	...
	{
		if(mpClient->getDisconnectedCause() == ExitGames::Chat::DisconnectCause::INVALID_AUTHENTICATION)
		{
