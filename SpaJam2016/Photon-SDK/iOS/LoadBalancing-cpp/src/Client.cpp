...
#	pragma warning(pop)
		...
		void Client::pingBestRegion(unsigned int pingsPerRegion)
		{
			...
			{
				mPeer.pingServer(mAvailableRegionServers[i], pingsPerRegion);
			...
			case OperationCode::GET_REGIONS:
				{
					...
					default:
						mPeer.opAuthenticate(mAppID, mAppVersion, SEND_AUTHENTICATE_ENCRYPTED, mAuthenticationValues, mAutoLobbyStats, mSelectedRegion=mAvailableRegions[0]);
						break;
					...
					case RegionSelectionMode::BEST:
						if(getRegionWithBestPing().length())
							mPeer.opAuthenticate(mAppID, mAppVersion, SEND_AUTHENTICATE_ENCRYPTED, mAuthenticationValues, mAutoLobbyStats, mSelectedRegion=getRegionWithBestPing());
						else
